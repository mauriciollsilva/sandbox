...
Se algo deu errado na instalação e você precisa limpar tudo para recomeçar, execute os comandos abaixo **um de cada vez**:

### 1. Remover os pacotes compilados

```bash
sudo rm -rf ~/src/Vrui
sudo rm -rf ~/src/Kinect
sudo rm -rf ~/src/SARndbox
```

### 2. Remover os arquivos instalados no sistema

```bash
sudo rm -rf /usr/local/share/Vrui*
sudo rm -rf /usr/local/lib/Vrui*
sudo rm -rf /usr/local/lib/libVrui*
sudo rm -rf /usr/local/lib/libKinect*
sudo rm -rf /usr/local/bin/KinectUtil
sudo rm -rf /usr/local/bin/RawKinectViewer
sudo rm -rf /usr/local/bin/SARndbox
sudo rm -rf /usr/local/bin/CalibrateProjector
sudo rm -rf /etc/udev/rules.d/69-Kinect.rules
```
...
### 4. Confirmar que está tudo limpo

```bash
ls ~/src/
ls /usr/local/share/ | grep -i vrui
ls /usr/local/bin/ | grep -iE "kinect|sandbox|calibrate"
```

Se os comandos acima não retornarem nada, está tudo limpo.

### 5. Rodar o instalador novamente
